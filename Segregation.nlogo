globals [
  ...
  max-value        ; maximum number of walls
]
...
patches-own [
  is-wall?  ; indicates whether a patch is a wall or not
]

to setup
  clear-all
  let max_value count patches with [not any? turtles-here and pcolor != black]
  if focus = "color" [
  ; create turtles on random patches.
  ask patches [
    set pcolor white
    set is-wall? false  ; initialize all patches as non-walls
    if random 100 < density [   ; set the occupancy density
      sprout 1 [
        ...
        set size 1  ; set initial size value for turtles
        ]
      ]
    ]
  ]
...
set max-value count patches with [not any? turtles-here and pcolor != black]
let wall-count max-value * wall-density / 100

; Generate random walls
repeat wall-count [
    let random-patch one-of patches with [not any? turtles-here and pcolor != black]
  ask random-patch [
    set is-wall? true
    set pcolor black
  ]
...
to find-new-spot
  rt random-float 360
  fd random-float 10
  if any? other turtles-here or is-wall? [ find-new-spot ] ; keep going until we find an unoccupied patch
...
to update-turtles
  ask turtles [
    ...
    if focus = "size" [
      ...
      set similar-nearby count (turtles-on neighbors) with [ size = [ size ] of myself ]
      set other-nearby count (turtles-on neighbors) with [ size != [ size ] of myself ]
    ]
...
<experiments>
  <experiment name="experiment" repetitions="10" runMetricsEveryStep="false">
    ...
    <enumeratedValueSet variable="focus">
      <value value="&quot;color&quot;"/>
    </enumeratedValueSet>
    <steppedValueSet variable="wall-density" first="0" step="10" last="100"/>
  </experiment>
  <experiment name="experiment2" repetitions="10" runMetricsEveryStep="false">
    ...
    <steppedValueSet variable="wall-density" first="0" step="10" last="90"/>
  </experiment>
  <experiment name="experiment2" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
